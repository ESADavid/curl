...
/* Create JSON payload for payroll validation */
char *validation_create_payroll_payload(const struct payroll_validation_request *request)
{
  ...
  escaped_frequency = json_escape_string(request->payroll_frequency);
...
/* Payroll validation API function */
CURLcode curl_validation_payroll(CURL *curl, const struct payroll_validation_request *request)
{
  ...
  payload = validation_create_payroll_payload(request);
  if(!payload)
